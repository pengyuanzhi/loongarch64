 ...
 * @brief 页帧号掩码
 */
#define _PFN_MASK    (~((_ULCAST_(1) << (PFN_PTE_SHIFT)) - 1UL) & \
                    ((_ULCAST_(1) << (_PAGE_PFN_END_SHIFT)) - 1UL))
...
#define LEVEL_SIZE_SHIFT(level)    \
    (LEVEL_FINAL_SIZE_SHIFT + (MAX_TABLE_LEVEL - (level)) * _512_SIZE_SHIFT)
...
#define GET_TABLE_OFF(level, addr)    \
    (((addr) >> LEVEL_SIZE_SHIFT(level)) & TABLE_LEVEL_MASK)
