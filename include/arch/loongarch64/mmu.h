...
#define __FIXMAP_op_S(i, size, set, op) \
    ((i) / 8U >= (size) ? 0             \
                        : (((unsigned long *)(set))[(i) / 8U / sizeof(long)] op(1UL << ((i) % (8U * sizeof(long))))))
...
#define __FIXMAP_op_func_S(func, op)                                                                          \
    static __inline void __FIXMAP_##func##_S(size_t __size, fixmap_set_t *__dest, const fixmap_set_t *__src1, \
                                             const fixmap_set_t *__src2)                                      \
    {                                                                                                         \
        size_t __i;                                                                                           \
        for (__i = 0U; __i < __size / sizeof(long); __i++)                                                    \
        {                                                                                                     \
            ((unsigned long *)__dest)[__i] = ((unsigned long *)__src1)[__i] op((unsigned long *)__src2)[__i]; \
        }                                                                                                     \
    }
...
#define BLOCK_DESC(attr, addr)                                                                                  \
    (((attr) & ENTRY_ATTRIB_MASK) | (((uint64_t)addr) & ENTRY_HUGE_ADDRESS_MASK) | _PAGE_HUGE | _PAGE_PRESENT | \
     __READABLE | __WRITEABLE | _PAGE_HGLOBAL)

/**
 * @brief 页描述符（用于普通页映射）
 */
#define PAGE_DESC(attr, addr) \
    (((attr) & ENTRY_ATTRIB_MASK) | (((uint64_t)addr) & ENTRY_ADDRESS_MASK) | _PAGE_PRESENT | __READABLE | __WRITEABLE)

/**
 * @brief 表描述符（扩展版，带属性）
 */
#define TABLE_DESC_EX(attr, next_table_point) (((attr) & ENTRY_ATTRIB_MASK) | ((next_table_point) & ENTRY_ADDRESS_MASK))
