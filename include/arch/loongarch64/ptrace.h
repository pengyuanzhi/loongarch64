/**
 * @file    ptrace.h
 * @brief   LoongArch64架构特定的ptrace定义
 * @author  Intewell Team
 * @date    2025-01-22
 * @version 1.0
 *
 * @details 本文件定义LoongArch64架构特定的ptrace寄存器映射
 *          - 寄存器偏移定义
 *          - CSR位域定义
 *          - 调试寄存器定义
 *          - Ptrace辅助函数
...
/**
 * @defgroup PtraceRegisters Ptrace寄存器定义
 * @{
 */

/**
 * @brief LoongArch64寄存器数量
 */
#define LOONGARCH64_NUM_REGS 32U
#define LOONGARCH64_NUM_FP_REGS 32U

/**
 * @brief 用户寄存器偏移（用于ptrace PEEKUSER/POKEUSER）
 */
 ...
 * @brief 特殊寄存器偏移
 */
#define LOONGARCH64_pc 32U       /**< 程序计数器 (ERA) */
#define LOONGARCH64_badvaddr 33U /**< 出错地址 */
#define LOONGARCH64_crmd 34U     /**< 当前模式寄存器 */
#define LOONGARCH64_prmd 35U     /**< 前一个模式寄存器 */
#define LOONGARCH64_euen 36U     /**< 扩展单元使能 */
#define LOONGARCH64_ecfg 37U     /**< 异常配置寄存器 */
#define LOONGARCH64_estat 38U    /**< 异常状态寄存器 */

/** @} */

/**
 * @defgroup CSRBits CSR位定义
 * @{
 */
 ...
 * @brief CRMD寄存器位定义
 */
#define CRMD_PLV (0x3UL << 0)  /**< 特权级别（0=内核, 3=用户） */
#define CRMD_DAIE (0x1UL << 2) /**< 地址异常中断使能 */
#define CRMD_WE (0x1UL << 3)   /**< 执行使能 */
#define CRMD_DATF (0x3UL << 4) /**< 数据访问类型 */
#define CRMD_DATM (0x3UL << 6) /**< 数据访问掩码 */
#define CRMD_DAV (0x1UL << 8)  /**< 数据访问有效性 */

/**
 * @brief PRMD寄存器位定义
 */
#define PRMD_PPLV (0x3UL << 0) /**< 前一个特权级别 */
#define PRMD_DATF (0x3UL << 4) /**< 前一个数据访问类型 */
#define PRMD_DATM (0x3UL << 6) /**< 前一个数据访问掩码 */
#define PRMD_DAV (0x1UL << 8)  /**< 前一个数据访问有效性 */

/**
 * @brief EUEN寄存器位定义
 */
#define EUEN_FPE (0x1UL << 0)   /**< 浮点单元使能 */
#define EUEN_SXE (0x1UL << 1)   /**< SIMD扩展使能 */
#define EUEN_ASXE (0x1UL << 2)  /**< 高级SIMD扩展使能 */
#define EUEN_LBTPE (0x1UL << 3) /**< 轻量级事务处理使能 */

/**
 * @brief ECFG寄存器位定义
 */
#define ECFG_LIE (0x1FFFUL << 0) /**< 局部中断使能 */

/**
 * @brief ESTAT寄存器位定义
 */
#define ESTAT_ECODE (0x3FUL << 0)      /**< 异常码 */
#define ESTAT_ESUBCODE (0x3FFUL << 16) /**< 异常子码 */
#define ESTAT_IS (0x1UL << 2)          /**< 中断栈 */

/** @} */

/**
 * @defgroup DebugRegisters 调试寄存器定义
 * @{
 */

/**
 * @brief LoongArch64调试CSR寄存器
 *
 * @note LoongArch64调试支持使用以下CSR寄存器：
 *       - PRCTL (0x80): 调试控制寄存器
 ...
 * @brief PRCTL寄存器位定义
 */
#define PRCTL_SS (0x1UL << 0)  /**< 单步使能 */
#define PRCTL_STE (0x1UL << 1) /**< 单步触发使能 */
#define PRCTL_ITE (0x1UL << 2) /**< 指令跟踪使能 */

/**
 * @brief 断点/观察点控制寄存器位定义
 */
#define DBG_CTRL_EN (0x1UL << 0)    /**< 断点/观察点使能 */
#define DBG_CTRL_MASK (0x3FUL << 2) /**< 地址掩码 */

/**
 * @brief 断点类型定义
 */
#define DBG_BPT_INST 0U  /**< 指令断点 */
#define DBG_BPT_DATA 1U  /**< 数据断点 */
#define DBG_BPT_RDWR 2U  /**< 读写断点 */
#define DBG_BPT_WRITE 3U /**< 写断点 */
...
/**
 * @brief LoongArch64特定的用户上下文
 *
 * @details 扩展标准user结构，添加架构特定字段
 */
struct loongarch64_user_context
{
    struct user regs; /**< 通用寄存器和FPU寄存器 */
    uint64_t orig_r4; /**< 原始系统调用参数 (r4/a0) */
};

/* ==================== 内联函数 ==================== */

/**
 * @defgroup PtraceHelpers Ptrace辅助函数
 * @{
 */
